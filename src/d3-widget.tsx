  ...
  return (
    <svg width={WIDTH} height={HEIGHT} ref={svgRef}>
      ...
      <g className="y-axis" transform={`translate(${margin.left},${margin.top})`} >
        ...
        <g className="tick">
          {yScale.ticks(10).map((d: any, i: number) => {
            const y = yScale(d);
            return (
              <g key={`g_${i}`} transform={`translate(0, ${y})`}>
                <line x1={0} x2={-6} y1={0} y2={0} stroke={"#000"} />
                <text x={-12} y={0} dy={"0.32em"} fontSize={12} textAnchor={"end"}>{d}</text>
              </g>
            );
          })}
        </g>
      ...
      {/* 柱子 */}
      <g transform={`translate(${margin.left}, ${margin.top})`} fill={"url(#linear-gradient)"} >
        {value.map((d, i) => {
          const x = xScale(d.x);
          const y = yScale(d.y);
          ...
          return (
            <g key={i}>
              ...
              <text x={(x || 0) + bandwidth / 2} y={y - 4} fontSize={12} textAnchor={"middle"} >
                {d.y.toFixed(0)}
              </text>
            </g>
          );
